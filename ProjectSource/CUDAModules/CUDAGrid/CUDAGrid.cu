...
/** @brief Domain width (in lattice units) for device kernels. */
__device__ size_t gpu_width;

/** @brief Domain height (in lattice units) for device kernels. */
__device__ size_t gpu_height;

/** @brief Radius used for computing Reynolds number and initialization. */
__device__ float gpu_r;

/** @brief Characteristic lattice velocity (uLB). */
__device__ float gpu_uLB;

/** @brief Reynolds number used for computing viscosity. */
__device__ float gpu_Re;

/** @brief Lattice viscosity derived from Re and uLB. */
__device__ float gpu_nulb;

/** @brief Relaxation parameter for the BGK collision model. */
